			  ...
			  HierarchyEntry &TopGrid, TopGridData &MetaData, ExternalBoundary &Exterior)
{
  ...
  float CollapseTestSphereDensity[MAX_SPHERES],
    ...
    CollapseTestSphereCloudPerturbation[MAX_SPHERES];
  ...
  for (sphere = 0; sphere < MAX_SPHERES; sphere++) {
    ...
    CollapseTestSphereCloudPerturbation[sphere] = 0.0;
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    if (sscanf(line, "CollapseTestSphereCloudPerturbation[%"ISYM"]", &sphere) > 0)
      ret += sscanf(line, "CollapseTestSphereCloudPerturbation[%"ISYM"] = %"ISYM, &sphere,
		    &CollapseTestSphereCloudPerturbation[sphere]);
  ...
  if (TopGrid.GridData->CollapseTestInitializeGrid(
	     ...
	     CollapseTestInitialFractionH2I, CollapseTestInitialFractionH2II,CollapseTestSphereCloudPerturbation) == FAIL) {
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    ...
    for (sphere = 0; sphere < CollapseTestNumberOfSpheres; sphere++) {
      fprintf(Outfptr, "CollapseTestSphereType[%"ISYM"] = %"ISYM"\n", sphere,
	      CollapseTestSphereType[sphere]);
      fprintf(Outfptr, "CollapseTestSphereCloudPerturbation[%"ISYM"] = %"ISYM"\n", sphere,
	      CollapseTestSphereCloudPerturbation[sphere]);
